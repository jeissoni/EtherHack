...
contract PirateShip {
    address public anchor = address(0);
    bool public blackJackIsHauled = false;



    // se debe llamar esta funcion desde un el constructor de un contrato
    // para poder cambiar la variable "blackJackIsHauled"
    function sailAway() public {
        
        require(anchor != address(0));
    ...
    function pullAnchor() public {
        require(anchor != address(0));
        (bool succes,) = anchor.call("");// raise the anchor if the ship is ready to sail away
    ...
    function assemble(uint[8] memory chunks) internal pure returns(uint code) {
